...
specification) and **vendor 2-byte opcodes** (prefix `0x0e`, Gigabyte-specific).
...
## Vendor commands (2-byte opcode, prefix `0x0e`)

Value meanings where impactful are listed; otherwise it is a normalized scale.

| Opcode | Control | `aorus-ctl` name | Range | Notes |
| --- | --- | --- | --- | --- |
| `0x0e 0x00` | Black equalizer | `black-equalizer` | 0–20 | shadow detail lift |
| `0x0e 0x03` | Colour mode | `colour-mode` | 0–3 | 0 cool · 1 normal · 2 warm · 3 user-defined |
| `0x0e 0x04` | RGB red gain | `rgb-red` | 0–100 | only effective in colour-mode 3 |
| `0x0e 0x05` | RGB green gain | `rgb-green` | 0–100 | only effective in colour-mode 3 |
| `0x0e 0x06` | RGB blue gain | `rgb-blue` | 0–100 | only effective in colour-mode 3 |
| `0x0e 0x07` | Gamma | `gamma` | 0–5 | 0 off · 1=1.8 · 2=2.0 · 3=2.2 · 4=2.4 · 5=2.6 |
| `0x0e 0x08` | Vibrance | `vibrance` | 0–20 | colour saturation boost |
| `0x0e 0x09` | Overdrive / MPRT | `overdrive` | 0–2 | 0 quality · 1 balance · 2 speed |
| `0x0e 0x0b` | Low blue light | `low-blue-light` | 0–10 | blue-light filter |
| `0x0e 0x0c` | FreeSync | `free-sync` | 0–1 | toggles adaptive sync |
| `0x0e 0x0e` | PBP / PIP mode | `pbp-pip-mode` | 0–2 | 0 off · 1 PIP · 2 PBP |
| `0x0e 0x0f` | PBP / PIP source | `pbp-pip-source` | 0–3 | 0 HDMI1 · 1 HDMI2 · 2 DP · 3 Type-C |
| `0x0e 0x10` | PBP / PIP switch | `pbp-pip-switch` | 0–9 | use 1 |
| `0x0e 0x13` | PIP audio switch | `pip-audio-switch` | 0–9 | use 1 |
| `0x0e 0x14` | PIP size | `pip-size` | 0–2 | 0 large · 1 medium · 2 small |
| `0x0e 0x15` | PIP location | `pip-location` | 0–3 | 0 top-left · 1 top-right · 2 bottom-left · 3 bottom-right |
| `0x0e 0x2c` | Picture mode | `picture-mode` | 0–8 | 0 standard · 1 FPS · 2 RTS/RPG · 3 Movie · 4 Reader · 5 sRGB · 6–8 custom |
| `0x0e 0x2d` | Input / source | `source` | 0–3 | 0 HDMI1 · 1 HDMI2 · 2 DP · 3 Type-C |
| `0x0e 0x2e` | Audio input | `audio-input` | 0–2 | 0 main · 1 PIP/PBP · 2 auto |
| `0x0e 0x30` | OSD display time | `osd-display-time` | 0–5 | 0=5s … 5=30s |
| `0x0e 0x31` | LED indicator | `led-indicator` | 0–2 | 0 always on · 1 always off · 2 standby on |
| `0x0e 0x69` | KVM switch | `kvm-switch` | 0–1 | **0 = USB-B · 1 = Type-C** |

---

## KVM switching (the special one)

KVM is a pure vendor command, `0x0e 0x69`: value `0` routes USB to the **USB-B**
...
> **Why both source and KVM exist:** `source` (0x0e 0x2d) selects the *video*
> input (which panel image is shown). `kvm-switch` (0x0e 0x69) selects which
...
- Opcodes prefixed `0x0e` are the vendor namespace; the *first* command byte is
  `0x0e` and the *value* is a single byte after the two-byte command word.
...
- Not every opcode is present on every panel. `volume` (0x62) and `audio-input`
  (0x0e 0x2e) only matter on panels with speakers/audio routing; on a
